...
- Project retargeted to `net10.0` with nullable + implicit usings, GA Microsoft.Extensions dependencies, and Dapr 1.16 packages.
- `Program.cs` now uses the minimal hosting model with primary-constructor DI, options binding, OpenTelemetry logging/metrics, and ADR-0005 health checks.
- Dockerfile moved to pinned GA images (`sdk:10.0.100`, `aspnet:10.0`) with a clean restore/publish workflow.
- Helm template + values gained consistent labels, env vars (`ASPNETCORE_URLS`, `DAPR_HTTP_PORT`), and `/healthz`/`/livez`/`/readyz` probes tied to the new health endpoints.
- Validation pipeline executed end-to-end (`upgrade-build-images.sh VirtualWorker`, `helm upgrade ... --wait`, `upgrade-validate.sh VirtualWorker`, worker smoke tests) with documentation captured in the session log.
