...
session: .claude/sessions/2025-11-13-0645-phase1a-remaining-dotnet10-upgrades.md
...
- Target framework, nullable, implicit usings, and LangVersion were upgraded to .NET 10, and all Microsoft.Extensions + Dapr packages now align with 10.0/1.16.0 requirements.
- Program.cs now uses `Host.CreateApplicationBuilder`, OpenTelemetry logging/metrics, and a DI-registered `VirtualCustomersWorker`; Serilog and blocking calls were removed.
- Dockerfile pins GA images only (`sdk:10.0.100`, `runtime:10.0`) and restores/publishes inside the build stage without deleting `global.json`.
- Helm chart + values files gained Dapr annotations, env-var driven options binding, and exec-based probes suitable for a console worker.
- `scripts/upgrade-build-images.sh VirtualCustomers`, `helm upgrade ... --wait`, `./scripts/upgrade-validate.sh VirtualCustomers`, and `scripts/run-virtualcustomers-smoke.sh` all executed successfully (validation logs archived in session file).
