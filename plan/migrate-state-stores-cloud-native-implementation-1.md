...
status: 'Planned'
tags: [infrastructure, migration, phase-0, state, dapr, cosmosdb, dynamodb, firestore, redis]
---
...
- **REQ-003**: Local development uses Redis 6.2.14 (Docker Compose)
...
- **CON-002**: State data must be exported/imported during migration
...
- **GOAL-001**: Export existing state data from Redis
...
### Implementation Phase 2: Create Cloud-Native State Store Components (Day 1-2)

- **GOAL-002**: Create Dapr component YAML files for each cloud
...
| TASK-207 | Create `manifests/branch/base/components/reddog.state.redis.yaml` (Redis 6.2.14 for local dev) | | |
...
- **DEP-003**: Workload Identity configured on clusters

## 5. Files

### Dapr Component Files

- **FILE-001**: `manifests/branch/base/components/reddog.state.redis.yaml` (local dev)
